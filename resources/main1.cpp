...
unsigned long id_function() { return (unsigned long)THREAD_ID; }
...
void CRYPTO_thread_cleanup() { int i;
    if (!mutex_buf)
        return;
...
// Funzione per ottenere le capacità del dispositivo
std::string get_device_capabilities(struct soap* soap, const std::string& hostname) {
  ...
  // Richiesta delle capacità del dispositivo
...
for (auto &element : get_xaddrs_vector()) {
    if (hostname == element.xaddr) {
        ...
        std::cout << "TID: " << gettid() 
                  ...
                  << std::endl;
        found = true; // Segnala che l'elemento è stato trovato e aggiornato
        break; // Esci dal ciclo una volta trovato
    }
}
xaddr_mutex.unlock();

// Aggiungi un nuovo elemento solo se non esiste già
if (!found) {
    xaddrs_t new_xaddr;
    ...
    xaddr_mutex.unlock();
    std::cout << "TID: " << gettid() 
              << " Media service endpoint: " << media_endpoint 
              << std::endl;
}
...
xaddr_mutex.lock();
    for (auto & element : get_xaddrs_vector()) {
        if (xaddr == element.xaddr) {
            std::cout << "TID: " << gettid() << " linea 579 " <<  std::endl; 
            //std::cout<< "TID: " << gettid()  << "Assigning media_endpoint: "<< element.media_endpoint << std::endl; 
            proxyMedia.soap_endpoint = element.media_endpoint.c_str();
            break;       
        }
...
xaddr_mutex.unlock();
   ...
   // std::string profile_token = GetProfilesResponse.Profiles[0]->token;
    ...
    std::cout << "TID: " << gettid() << "linea 621 " <<  std::endl; 
    ...
    std::cout << "TID: " << gettid() << "linea 635 " <<  std::endl; 
    if (GetSnapshotUriResponse.MediaUri) {
        std::cout << "TID: " << gettid() << "linea 637 " <<  std::endl; 
   ...
   // std::cout << "TID: " << gettid() << "proxyEvent.soap_endpoint: " << proxyEvent.soap_endpoint << std::endl;
    ...
    for (auto item : pullResponse.wsnt__NotificationMessage) {
        if (item->Topic && item->Topic->__any) {
            ...
            std::cout << "TID: "<< gettid() << "ciao" << std::endl;
